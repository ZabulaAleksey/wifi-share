...
Сервер принимает `localhost`, конкретный private/loopback IP или явный opt-in `0.0.0.0` для доверенной LAN; неявный wildcard и динамический порт запрещены. Временные загрузки хранятся в зарезервированном `.wifi-share-tmp` с правами `0700`, не видны API и атомарно переименовываются только после `Sync` и закрытия. Это уменьшает случайную сетевую экспозицию и исключает выдачу partial files из share root.
